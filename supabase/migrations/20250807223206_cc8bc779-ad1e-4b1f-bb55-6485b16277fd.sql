-- Update the function to use the Make.com webhook URL from Supabase secrets
...
BEGIN
  ...
  IF OLD.status IN ('registered', 'waitlist') AND NEW.status = 'cancelled' THEN
    ...
    webhook_url := current_setting('app.settings.make_waitlist_webhook_url', true);
          ...
          IF FOUND THEN
            ...
            IF webhook_url IS NOT NULL AND webhook_url != '' THEN
              BEGIN
                ...
                RAISE LOG 'Webhook sent for waitlist promotion: user_id=%, course_id=%, webhook_url=%', waitlist_rec.user_id, NEW.course_id, webhook_url;
