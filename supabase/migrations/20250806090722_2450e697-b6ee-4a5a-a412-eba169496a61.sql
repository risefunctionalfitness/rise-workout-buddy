-- Fix trigger problem: Remove redundant triggers and consolidate functionality
-- There are multiple conflicting triggers calling advance_waitlist

-- First, drop all existing triggers on course_registrations (except foreign key triggers)
DROP TRIGGER IF EXISTS course_registration_insert_trigger ON public.course_registrations;
DROP TRIGGER IF EXISTS course_registration_update_trigger ON public.course_registrations; 
DROP TRIGGER IF EXISTS course_registration_change_trigger ON public.course_registrations;

-- Drop the old trigger function that was causing conflicts
DROP FUNCTION IF EXISTS public.handle_course_registration_change();

-- Update the consolidated trigger function to ensure proper UUID casting
