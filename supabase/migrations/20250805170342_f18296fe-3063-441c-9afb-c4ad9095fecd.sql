-- Debug function to check course registration validation
...
BEGIN
    ...
    IF _user_membership_type = 'Basic Member' THEN
        _weekly_count := get_weekly_registrations_count(user_id_param, _course_date);
...
-- Test the function with Björn's data
SELECT * FROM debug_can_user_register_for_course(
    (SELECT user_id FROM profiles WHERE display_name ILIKE '%björn%' LIMIT 1),
    (SELECT id FROM courses WHERE course_date = '2025-08-08' LIMIT 1)
);