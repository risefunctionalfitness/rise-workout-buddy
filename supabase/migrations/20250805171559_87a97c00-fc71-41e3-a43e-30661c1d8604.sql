-- Completely recreate the functions with explicit type casting
...
BEGIN
    ...
    IF user_membership_type = 'Basic Member' THEN
        -- Use function with explicit type casting
        weekly_count := get_weekly_registrations_count(user_id_param::uuid, CURRENT_DATE::date);
...
-- Fix the debug function too with explicit type casting
...
BEGIN
    ...
    IF _user_membership_type = 'Basic Member' THEN
        -- Use function with explicit type casting
        _weekly_count := get_weekly_registrations_count(user_id_param::uuid, CURRENT_DATE::date);
