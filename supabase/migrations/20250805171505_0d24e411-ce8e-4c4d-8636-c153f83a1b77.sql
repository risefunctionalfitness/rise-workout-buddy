-- Fix the functions to use the correct get_weekly_registrations_count signature with 2 parameters
...
BEGIN
    ...
    IF user_membership_type = 'Basic Member' THEN
        -- Use function with correct signature: user_id and check_date
        weekly_count := get_weekly_registrations_count(user_id_param, CURRENT_DATE);
...
BEGIN
    ...
    IF _user_membership_type = 'Basic Member' THEN
        -- Use function with correct signature: user_id and check_date
        _weekly_count := get_weekly_registrations_count(user_id_param, CURRENT_DATE);
