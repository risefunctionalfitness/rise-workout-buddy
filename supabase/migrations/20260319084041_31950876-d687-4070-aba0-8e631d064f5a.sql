-- Add 'admin_cancelled' to the status check constraint
...
CHECK (status = ANY (ARRAY['registered','waitlisted','cancelled','waitlist','waitlist_cancelled','admin_cancelled']));

-- Update handle_membership_limits to handle admin_cancelled like cancelled
...
BEGIN
    ...
    IF TG_OP = 'UPDATE' AND OLD.status IN ('cancelled', 'waitlist_cancelled', 'admin_cancelled') AND NEW.status IN ('registered', 'waitlisted') THEN
        SELECT membership_type INTO user_membership_type
        FROM public.profiles WHERE user_id = NEW.user_id;
    ...
    IF TG_OP = 'UPDATE' AND OLD.status IN ('registered', 'waitlisted') AND NEW.status IN ('cancelled', 'waitlist_cancelled', 'admin_cancelled') THEN
        SELECT membership_type INTO user_membership_type
        FROM public.profiles WHERE user_id = NEW.user_id;
...
-- Update process_waitlists_on_cancellation to also trigger on admin_cancelled
...
BEGIN
  IF NEW.status IN ('cancelled', 'admin_cancelled') AND (OLD.status IS NULL OR OLD.status NOT IN ('cancelled', 'admin_cancelled')) THEN
